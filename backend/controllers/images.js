...
exports.transaction = async (req, res) => {
  try {
    ...
    const resp = await web3.eth.getTransactionReceipt(
      "0x1cc752c0683b5f9b85c1ef60ba207503cee7c2444114d60502d34cb2d0c320e3"
    );
    res.send(resp);
  } catch (err) {
    res.status(500).send(err.message);
...
exports.getData = async (req, res) => {
  ...
  const accounts = await web3.eth.getAccounts();
  try {
    const resp = await SavingContract.methods.getHash(req.params.name).send({
      from: accounts[0]
    });
